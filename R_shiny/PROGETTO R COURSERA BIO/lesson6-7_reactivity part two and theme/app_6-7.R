...
library(DT)
library(GeneBook)
library(shinythemes) # librerai facile per modificare applicazioni 
...
ui <- fluidPage( theme = "mytheme.css",# posso cambaire direttamente anche il file di mytheme.css per fare modifiche al tema, se si cambia da mytheme bisogna tistartare seesione di R seno rimane la modifica precedente 
                 # shinythemes::themeSelector(), # funzione che non utilizza themi ma libreria shinytheme 
                 # theme = shinytheme("darkly"), # dopo aver scelto con themeSelector() quello che ci sta meglio con questa funzione theme selezioniamo il tema che voliamo
    ...
    sidebarLayout(
        ...
        sidebarPanel( tags$style(".well {background-color:black;}"), # serve per cambiare la sidebar rendendola nera ,
            selectInput(
                inputId ="G_groups",
...
server <- function(input, output) {
 ...
 #da qui in poi è per presentare come output file html, plot e table

    full_file_name <-reactive(paste("./inc/", input$G_groups, ".csv", sep = ""))

    output$downloadData <- downloadHandler( # la funzione downloadhandler ha bisongo di un filename e content, in questo caso ci da codice del file da quel che ho capito 
    ...
    # renderDT() from DT library is a replacement for Shiny renderDataTable() per table riguardanti i nositri geni
    output$datatable1 <- renderDT(datatable(g1))
    output$datatable2 <- renderDT(datatable(g2))
    output$datatable3 <- renderDT(datatable(g3))

    #in questo caso in base a cosa scegliamo nella sezione C della UI ci può ridare pagine html oppure table oppure altro in abase a quello che scegliamo nel codice 
